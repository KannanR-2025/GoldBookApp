...
class MetalReceiptEntryScreen extends ConsumerStatefulWidget {
  final int? transactionId;
  const MetalReceiptEntryScreen({super.key, this.transactionId});

  @override
  ConsumerState<MetalReceiptEntryScreen> createState() =>
      _MetalReceiptEntryScreenState();
}

class _MetalReceiptEntryScreenState
    extends ConsumerState<MetalReceiptEntryScreen> {
  final _formKey = GlobalKey<FormState>();

  // Header State
  final String _type = 'MetalReceipt';
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildSectionCard(
                                title: 'Party & Date',
                                child: _buildHeaderSection(parties, partyType),
                              ),
                              const SizedBox(height: 24),
                              // Items Section
                              _buildSectionCard(
                                title: 'Items',
                                child: _buildItemsSection(itemsAsync),
                              ),
                              const SizedBox(height: 24),
                              // Totals Section
                              _buildSectionCard(
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              onPressed: _save,
                              child: const Text('Save Metal Receipt'),
                            ),
                          ],
                        ),
                      ),
  ...
  Widget _buildHeaderSection(List<Party> parties, String partyType) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
            ...
            if (_selectedPartyId != null)
              Builder(
                builder: (context) {
                  ...
                  if (party.id == -1) return const SizedBox();
                  return Container(
            ...
            ...List.generate(_lines.length, (index) {
              return Padding(
                padding: EdgeInsets.only(
                  bottom: index < _lines.length - 1 ? 12 : 0,
                ),
                child: _buildLineItem(index, inventoryItems),
              );
            }),
          ],
        );
