...
class _PurchasesScreenState extends ConsumerState<PurchasesScreen> {
        ...
        data: (allTransactions) {
          ...
          final filteredPurchases = _searchQuery.isEmpty
              ...
              : purchases.where((purchase) {
                  final partyName = purchase.party.name.toLowerCase();
                  final invoiceNo =
                      purchase.transaction.transactionNumber?.toLowerCase() ??
                      '';
            ...
            return Column(
              children: [
                ...
                const Expanded(
                  child: Center(
                    child: Text(
                      'No purchase transactions found. Create a new purchase to get started.',
                    ),
                  ),
                ),
              ],
          ...
          return Column(
            children: [
              _buildSearchAndActions(),
              Expanded(child: _buildTable(filteredPurchases)),
            ],
          );
        },
  ...
  Widget _buildTable(List<TransactionWithParty> purchases) {
    return Container(
        ...
        child: SingleChildScrollView(
          child: _ProfessionalDataTable(
            ...
            rows: purchases.asMap().entries.map((entry) {
              ...
              final rowNumber = purchases.length - index;
              return _TableRow(
                cells: [
                  ...
                  _TableCell(purchase.transaction.transactionNumber ?? '-'),
                  _TableCell(
                    purchase.party.name,
                    isLink: true,
                    onTap: () {
                  ...
                  _TableCell(
                    FutureBuilder<Map<String, dynamic>>(
                      future: _getTransactionLineStats(purchase.transaction.id),
                  ...
                  _TableCell(
                    NumberFormat.currency(
                      symbol: '₹',
                    ).format(purchase.transaction.totalAmount),
                    alignment: TextAlign.right,
                    isBold: true,
                  ),
                  _TableCell(
                    PopupMenuButton<String>(
                      ...
                      onSelected: (value) async {
                        if (value == 'view') {
                          context.push(
                            '/purchases/edit/${purchase.transaction.id}',
                          );
                        } else if (value == 'edit') {
                          context.push(
                            '/purchases/edit/${purchase.transaction.id}',
                          ...
                          if (confirmed) {
                            try {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                    'Purchase from ${purchase.party.name} deleted',
                                  ),
                ...
                onTap: () {
                  context.push('/purchases/edit/${purchase.transaction.id}');
                },
              );
            }).toList(),
          ),
